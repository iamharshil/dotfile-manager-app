...
export default function EditConfigForm({
  ...
  const [filePath, setFilePath] = useState(
    initial?.filePath ?? (tool === "vscode" ? "settings.json" : "init.lua"),
  );
  const [language, setLanguage] = useState<Language>(
    initial?.language ??
      (tool === "vscode" ? "json" : tool === "neovim" ? "lua" : "json"),
  );
    ...
    >
      <GlassCard variant="purple" className="p-4 sm:p-6">
            ...
            <div className="flex items-center justify-between rounded-md border bg-background/60 p-3">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Make this config visible to others
                </p>
              </div>
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-end">
          ...
          >
            <a href={`/dashboard/configs/${id}`}>Cancel</a>
          </Button>

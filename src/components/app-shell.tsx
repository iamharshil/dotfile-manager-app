...
export default function AppShell({ title, description, action, children, className, session }: AppShellProps) {
			...
			<main className={cn("mx-auto max-w-6xl px-4 py-4 md:py-8", className)}>
				...
				>
					<div className="min-w-0">
						<h1 className="text-pretty text-xl font-semibold tracking-tight md:text-2xl">{title}</h1>
					...
					<div className="flex items-center gap-1">
						<nav aria-label="Primary" className="hidden items-center gap-1 md:flex">
							...
							{session?.user ? null : (
								<Button asChild className="ml-1 rounded-full" variant="default">
